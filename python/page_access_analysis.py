...
def process(table_name,key_type,SQL_count):
    ...
    SQL+='(页面 varchar(255) not null,'
    SQL+='时间 %s not null,'%key_type
    SQL+='访问量 bigint not null,'
    SQL+='primary key(页面,时间))'
    print(SQL)
    cursor.execute(SQL)
    dbc.commit()

    SQL="SELECT DISTINCT 所在页面 FROM 事件记录"
    print(SQL)
    cursor.execute(SQL)
    ...
    for page in pages:
        ...
        SQL="SELECT max(时间) FROM %s WHERE 页面='%s'"%(table_name,page)
        print(SQL)
        cursor.execute(SQL)
        ...
        #从原始数据表中选出数据统计并插入
        SQL=SQL_count%(event_type_name['访问页面'],page,str(last_count))
        print(SQL)
        cursor.execute(SQL)
        results=cursor.fetchall()
        SQL_f="INSERT INTO %s(页面,时间,访问量)VALUES('%s','%s','%d')"
...
process('小时页面访问量','datetime',SQL_count)
